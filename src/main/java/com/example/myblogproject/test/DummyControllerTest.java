...
public class DummyControllerTest {
    ...
    public User updateUser(@PathVariable int id, @RequestBody User requestUser){
        //@Requestbody : json 데이터를 요청 -> Java Object(MessageConverter의 Jackson라이브러리가 변환해서 받아준다.
        ...
        //userRepository.save(user); //save는 insert 할 때 쓰는 메서드
    ...
    public User detail(@PathVariable int id){
        //user/4을 찾으면 내가 DB에서 못 찾아오게되면 user가 null이 될 것 아냐?
        ...
        //만약, 자바 오브젝트를 리턴하게 되면 MessageConverter가 Jackson이라는 라이브러리 호출해서
    ...
    public String join(@RequestBody User user){ //key = value (약속된 규칙)
